 ...
 * ------------------------------------------------
 * Four prompt templates called by Supabase Edge Functions against the Anthropic API.
 * Self-contained module: declares its own local types so it compiles before
 * /lib/metals.ts and /types/database.ts are reconciled in Wave 3.
 *
 * Run directly for a live smoke-test:
 *   ANTHROPIC_API_KEY=... npx tsx lib/ai/prompts.ts
 */
...
// ============================================================
// LOCAL TYPES
// (will be reconciled with /lib/metals.ts + /types/database.ts in Wave 3)
// ============================================================
...
// ============================================================
// MAIN — live smoke-test gate
// Run: ANTHROPIC_API_KEY=... npx tsx lib/ai/prompts.ts
// ============================================================
...
async function main(): Promise<void> {
  ...
  // ── Test 1: Relevance ─────────────────────────────────────
  console.log(hr);
  console.log('PROMPT 1 — RELEVANCE FILTER');
  const relevanceInput: RelevanceInput = {
    headline: 'Codelco copper output falls 8% in Q1 on Chuquicamata smelter shutdown',
    summary:
      "Chile's state miner Codelco reported first-quarter copper production of 318,000 tonnes, down from 346,000 tonnes a year earlier, after a fire damaged the Chuquicamata smelter in February.",
    source: 'Reuters',
  };
  console.log('Input:', relevanceInput.headline);
  const r1 = await client.messages.create({
    ...
    messages: [{ role: 'user', content: RELEVANCE_PROMPT.buildUserMessage(relevanceInput) }],
  });
  const raw1 = r1.content[0].type === 'text' ? r1.content[0].text : '';
  console.log('Raw output :', raw1);
  console.log('Parsed     :', parseRelevanceResponse(raw1));

  // ── Test 2: Sentiment + Tagging ───────────────────────────
  console.log(hr);
  console.log('PROMPT 2 — SENTIMENT + METAL TAGGING');
  const classifyInput: ClassifyInput = {
    headline: 'LME on-warrant aluminium stocks reach six-month high as European demand softens',
    summary:
      'Aluminium inventories on LME warrant rose 14% week-on-week to 412,000t, the highest since September 2024, as weaker-than-expected European manufacturing data dampened restocking appetite.',
    source: 'Metal Bulletin',
  };
  console.log('Input:', classifyInput.headline);
  const r2 = await client.messages.create({
    ...
    messages: [{ role: 'user', content: CLASSIFY_PROMPT.buildUserMessage(classifyInput) }],
  });
  const raw2 = r2.content[0].type === 'text' ? r2.content[0].text : '';
  console.log('Raw output :', raw2);
  console.log('Parsed     :', parseClassifyResponse(raw2));

  // ── Test 3: Morning Brief ─────────────────────────────────
  console.log(hr);
  console.log('PROMPT 3 — MORNING BRIEF');
  const briefInput: BriefInput = {
    as_of: '2025-04-22T06:00:00Z',
    price_moves: [
      { metal: 'copper',    cash_price: 9847,  change_pct:  1.2,  unit: 'tonne'    },
      { metal: 'aluminium', cash_price: 2341,  change_pct: -0.4,  unit: 'tonne'    },
      { metal: 'nickel',    cash_price: 15920, change_pct: -1.1,  unit: 'tonne'    },
      { metal: 'gold',      cash_price: 2931,  change_pct:  0.6,  unit: 'troy_oz'  },
    ],
    arb_snapshot: {
      lme_copper_usd_tonne:   9847,
      comex_copper_usd_tonne: 9972,
      spread_usd:              125,
      spread_pct:             1.27,
    },
    top_headlines: [
      {
        headline:  'Codelco declares force majeure at Chuquicamata following underground collapse',
        source:    'Reuters',
        metals:    ['copper'],
        sentiment: 'bullish',
      },
      {
        headline:  'Indonesian nickel ore export quota cut by 20% for Q2 amid processing capacity shortfall',
        source:    'Bloomberg',
        metals:    ['nickel'],
        sentiment: 'neutral',
      },
      {
        headline:  'Gold advances to six-week high as safe-haven demand intensifies on US-China trade tensions',
        source:    'Financial Times',
        metals:    ['gold'],
        sentiment: 'bullish',
      },
    ...
    upcoming_events: [
      { date: '2025-04-22', event: 'US Federal Reserve rate decision (FOMC)', impact: 'high'   },
      { date: '2025-04-23', event: 'China industrial output data — March',     impact: 'high'   },
    ],
  };
  const r3 = await client.messages.create({
    model: BRIEF_PROMPT.model,
    max_tokens: BRIEF_PROMPT.max_tokens,
    temperature: BRIEF_PROMPT.temperature,
    system: BRIEF_PROMPT.system,
    messages: [{ role: 'user', content: BRIEF_PROMPT.buildUserMessage(briefInput) }],
  });
  const raw3 = r3.content[0].type === 'text' ? r3.content[0].text.trim() : '';
  console.log('Output:\n');
  console.log(raw3);

  // ── Test 4: Per-commodity Explainer ───────────────────────
  console.log(hr);
  console.log('PROMPT 4 — PER-COMMODITY EXPLAINER');
  const explainerInput: ExplainerInput = {
    metal:                  'copper',
    current_price:           9847,
    unit:                   'tonne',
    change_pct:              1.2,
    cash_to_3m_spread:      -42,
    lme_stock:               138500,
    cancelled_warrants_pct:  31,
    relevant_headlines: [
      {
        headline:  'Codelco declares force majeure at Chuquicamata following underground collapse',
        source:    'Reuters',
  ...
  console.log(`Input: ${explainerInput.metal} ${explainerInput.change_pct > 0 ? '+' : ''}${explainerInput.change_pct}%`);
  const r4 = await client.messages.create({
    model: EXPLAINER_PROMPT.model,
    max_tokens: EXPLAINER_PROMPT.max_tokens,
    temperature: EXPLAINER_PROMPT.temperature,
    system: EXPLAINER_PROMPT.system,
    messages: [{ role: 'user', content: EXPLAINER_PROMPT.buildUserMessage(explainerInput) }],
  });
  const raw4 = r4.content[0].type === 'text' ? r4.content[0].text : '';
  console.log('Raw output :', raw4);
  console.log('Parsed     :', JSON.stringify(parseExplainerResponse(raw4), null, 2));

  console.log(hr);
  console.log('All four prompts tested successfully.');
}
